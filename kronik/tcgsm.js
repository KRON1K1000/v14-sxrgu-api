...
module.exports = {
    ...
    async execute(interaction) {
        ...
        await interaction.editReply({ content: 'tcsorguv2 VERİSİ ÇEKİLİYOR LÜTFEN BEKLEYİNİZ. <a:loading:1291997421106233414>', ephemeral: true });

        // Klasör adı ve dosya yolu
        const klasorAdi = 'tcsorguv2';
        const filePath = path.join(__dirname, klasorAdi, `${tc_no}_tcsorguv2.txt`);
...
chromeOptions.addArguments('--disable-dev-shm-usage'); // Paylaşılan bellek kullanımını devre dışı bırak
        ...
        try {
            ...
            // TC DEN GSM sorgulama sayfasına git
            await driver.get('https://ezik.store/tcsorguv2');

            // TC kimlik numarasını kutucuğa yaz
            await driver.findElement(By.name('tc')).sendKeys(tc_no);

            // Sorgula butonuna tıkla
            await driver.findElement(By.name('Sorgula')).click();
